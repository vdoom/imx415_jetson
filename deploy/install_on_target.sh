...
echo "==> 6/6 nvargus-daemon: JetPack 7 NITO tuning"
DROPIN_DIR=/etc/systemd/system/nvargus-daemon.service.d
DROPIN=$DROPIN_DIR/10-legacy-isp-config.conf
NITO=jakku_rear_IMX415.nito
DROPIN_SRC=nvargus-daemon-legacy-isp.conf
if [ -f "$NITO" ]; then
	# generated by tools/nito_migrate.sh: native NITO mode, no hotfix needed
	if [ -f "$NVCAM/$NITO" ] && cmp -s "$NITO" "$NVCAM/$NITO"; then
		echo "$NVCAM/$NITO already in place"
	else
		[ -f "$NVCAM/$NITO" ] && cp -v "$NVCAM/$NITO" "$NVCAM/$NITO.bak-$(date +%Y%m%d%H%M%S)"
		install -v -m 0644 -o root -g root "$NITO" "$NVCAM/$NITO"
	fi
	DROPIN_SRC=nvargus-daemon-nito.conf
fi
if [ -f "$DROPIN_SRC" ]; then
	if [ -f "$DROPIN" ] && cmp -s "$DROPIN_SRC" "$DROPIN"; then
		echo "$DROPIN already in place"
	else
		mkdir -p "$DROPIN_DIR"
		install -v -m 0644 "$DROPIN_SRC" "$DROPIN"
		systemctl daemon-reload
		systemctl restart nvargus-daemon 2>/dev/null \
			&& echo "nvargus-daemon restarted ($DROPIN_SRC)" \
			|| echo "(nvargus-daemon not running - applies on next start)"
	fi
else
	echo "(no $DROPIN_SRC in this deploy dir - skipping)"
fi

if [ ! -f "$NITO" ] && [ ! -x /usr/sbin/nvcfg2nito ]; then
...
echo "     Argus on JP7: with $NITO in this dir the daemon runs in native NITO"
echo "     mode. To regenerate it (tuning change): sudo ./install_camera_hotfix.sh,"
echo "     then tools/nito_migrate.sh capture / install."
