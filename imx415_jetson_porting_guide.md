...
Протоколи збору даних: target — `passport.md` (2026-07-06); Фаза A на RPi5 —
`rpi5_imx415_data.md` (2026-07-05, **містить поправки до §5.0/§6.2 — зокрема
15 fps @ 2-lane замість 30 і гілку регістрів rpi-6.12.y**).

| Параметр | Значення | Фаза |
|---|---|---|
| L4T (`nv_tegra_release`) | R36.5.0, GCID 43688277 (JP 6.2.2), ядро 5.15.185-tegra | 1.4 |
| Кварц модуля | **37.125 МГц** (clk_summary на RPi5) | 2.2 |
| I2C-адреса | **0x37** (не 0x1a! страп модуля; підтверджено на RPi5 і на Jetson читанням регістрів) | 2.3 |
| Bayer-порядок | **GBRG** (`SGBRG10_1X10`), 10 біт | 2.3 |
| Розміри кадру reference-драйвера | **3864×2192**, єдиний режим, **15 fps** @ 2-lane (не 30! див. rpi5_imx415_data.md §2.2) | 2.3 |
| Порт на девкіті | **CAM1** (i2c-9 = mux chan 1; reset = PAC.00 = gpiochip0 line 138, active-high release) | 3.2 |
| Номер I2C-шини на Jetson | **9** | 3.2 |
| Донорський overlay-файл | `tegra234-p3767-camera-p3768-imx219-C.dts` | 6.1 |
| tegra_sinterface | **serial_c** (звірено з живим DT; увага: cam0 у цьому DT = serial_b, не serial_a) | 6.1 |
| link_freq для INCK 37.125 | **445.5 МГц** (підтверджено `link_frequency` ctrl на RPi) | 5.3.6 |
| pix_clk_hz (розрахунок) | 178 200 000 (але line_length = **5280**, framerate **15 fps** — rpi5_imx415_data.md §2.3) | 6.2 |
| fourcc формату на VI | | 8.1 |
| Полярність IR-CUT | ще не знято — потрібен дріт на GPIO (rpi5_imx415_data.md §6) | 2.5 |
