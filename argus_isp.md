...
boot-persistence setup lives there too (label `"IMX415"`, shared with the
CUDA bridge — the two docs write the same modprobe.d file). Known flake:
cheese failed loopback discovery on this target 2026-07-10 while guvcview
and gst v4l2src worked — start the producer first, launch the app fresh;
guvcview is the reliable fallback.
